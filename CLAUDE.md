...
- Release flow: bump `src/lib/releaseNotes.ts` (team-facing changelog at `/admin/release-notes`), commit, deploy with `./deploy.sh` (prod = DigitalOcean droplet; verify `curl http://206.189.200.138:3000/login` → 200).
